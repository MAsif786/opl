...
class DomainConfig(BaseModel):
    ...
    dimensions: list[str] = Field(
        ...,
        min_length=1,
        description="Ordered list of state dimension names (e.g., ['stock', 'demand'])"
    )

    physics_rule: str = Field(
        "logistics_basic",
        description="Name of the physics rule to use from the Rule Registry"
    )

    rule_params: dict[str, float] = Field(
        default_factory=dict,
        description="Physical constants for the physics rule (e.g., lead_time)"
    )

    actions: list[str] = Field(
        ...,
        min_length=1,
        description="List of allowed action names (e.g., ['reorder', 'transfer'])"
    )

    cost_params: dict[str, float] = Field(
        default_factory=dict,
        description="Key-value pairs for the domain's cost function"
    )
